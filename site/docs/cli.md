...
`--template` (POTX) and `--embed-fonts` are **documented CLI flags but not implemented yet**. See [limitations.md](./limitations.md).

Always compile with a current `yumiamd` (v0.1.31+). An older global `yumia` on your PATH can silently produce stale PDF/PPTX output.
